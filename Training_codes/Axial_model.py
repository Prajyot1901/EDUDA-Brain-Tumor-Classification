...
# Logging Config
# -----------------------
log_path = r"Transfer learning\UDA_MRI\Outputs\Axial_log.log"
...
MRI1_ROOT = r"Transfer learning\UDA_MRI\Slice_Separated_Dataset\Axial\Source"
MRI2_ROOT = r"Transfer learning\UDA_MRI\Slice_Separated_Dataset\Axial\Target"
...
# Dataset
# -----------------------
def load_dataset(folder_root, labels, image_size, flip=False):
    images, targets = [], []
...
def evaluate_mri2_confusion(model, loader, device, labels=LABELS):
    ...
    # plt.ylabel("True")
    # plt.title("MRI-2 Confusion Matrix")
    # plt.show()
...
if __name__ == "__main__":
    mri1_images, mri1_labels = load_dataset(MRI1_ROOT, LABELS, IMAGE_SIZE, flip=False)
    mri2_images, mri2_labels = load_dataset(MRI2_ROOT, LABELS, IMAGE_SIZE, flip=True)
    ...
    best_score = 0.0
    best_model_path = r"Transfer learning\UDA_MRI\Model weights\Axial_weights.pth"
    logs_path = r"Transfer learning\UDA_MRI\Outputs\Axial_training.pth"
    ...
    # Save logs + final report
    torch.save({
        "phase2_logs": phase2_logs,
        "best_score": best_score,
        "final_classification_report": final_report
    }, logs_path)

    logger.info(f"Phase 2 training logs and final report saved at {logs_path}")
