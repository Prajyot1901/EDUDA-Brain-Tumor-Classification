...
# Logging Config
# -----------------------
log_path = r"UDA_MRI\Outputs\sagittal_log.log"

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(log_path, mode="w"),  # save logs to file
...
MRI1_ROOT = r"UDA_MRI\Slice_Separated_Dataset\Sagittal_dataset\Source"
MRI2_ROOT = r"UDA_MRI\Slice_Separated_Dataset\Sagittal_dataset\Target"
...
# -----------------------
# Model: Frozen ResNet50 + 4 FC layers
# -----------------------
...
def train_one_epoch(model, source_loader, target_loader, optimizer, criterion, device):
    ...
    for imgs_s, labels_s in tqdm(source_loader, desc="Train batches"):
        ...
        loss = loss_cls + 1e-3*loss_coral*0 # coral disabled now
...
# -----------------------
# Feature Extraction after 3rd FC
# -----------------------
...
def compute_mmd_loss(Xs, Xt, kernel_fn=None):
    ...
    Ktt = kernel_fn(Xt, Xt)
    Kst = kernel_fn(Xs, Xt)
    return torch.mean(Kss) + torch.mean(Ktt) - 2.0 * torch.mean(Kst)
...
def final_classification_report(model, loader, device, labels=LABELS):
    ...
    logger.info(classification_report(all_labels, all_preds, target_names=labels))
    return report
...
if __name__ == "__main__":
    ...
    best_score = 0.0  
    best_model_path = r"UDA_MRI\Model weights\Sagittal_weights.pth"
    best_log_path   = r"UDA_MRI\Outputs\Sagittal_training.pth"
    ...
    training_logs = {"epochs": [], "train_loss": [], "train_acc": [], "val_acc": [], "test_acc": []}
    ...
    for epoch in range(1, PHASE_2_EPOCHS+1):
        ...
        if score > best_score:
            ...
            torch.save(training_logs, best_log_path)
    ...
    # Final Evaluation
    acc, cm = evaluate_mri2_confusion(model, mri2_loader, DEVICE)
    report = final_classification_report(model, mri2_loader, DEVICE)

    # Save final logs with classification report
    training_logs["final_test_acc"] = acc
    training_logs["final_classification_report"] = report
    torch.save(training_logs, best_log_path)
    logger.info("Saved final training logs with classification report.")
